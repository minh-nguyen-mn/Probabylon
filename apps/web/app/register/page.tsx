...
export default function RegisterPage() {
  ...
  async function handleCompleteSubmit(e: FormEvent) {
    ...
    setLoading(true);
    try {
      const resp = await apiGoogleLogin(googleIdToken, normalizedUsername, completeData.password, normalizedName);
      setAuth(resp.access_token, resp.user);
      window.location.href = "/";
    } catch (err: any) {
      ...
      <div className="relative w-full max-w-md">
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/80 p-8 shadow-2xl backdrop-blur-xl">
          <div className="mb-8 text-center">
            ...
            <h1 className="mb-1 text-2xl font-bold text-white">{showCompleteProfile ? "Hoàn tất thông tin" : "Tạo tài khoản"}</h1>
            <p className="text-sm text-zinc-400">
              {showCompleteProfile ? "Bổ sung vài thông tin để hoàn tất đăng ký." : "Tạo tài khoản mới để tham gia dự báo và giao dịch xác suất."}
            </p>
          </div>
          ...
          {showCompleteProfile ? (
            <form onSubmit={handleCompleteSubmit} className="space-y-4">
              ...
              <button type="button" onClick={() => setShowCompleteProfile(false)} className="w-full py-2 text-sm text-zinc-500 transition-colors hover:text-white">
                Hủy bỏ
              </button>
            </form>
          ) : (
            <>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Field label="Tên đăng nhập" value={username} onChange={setUsername} placeholder="ten_dang_nhap" />
                <Field label="Email" type="email" value={email} onChange={setEmail} placeholder="ban@email.com" />
                <Field label="Mật khẩu" type="password" value={password} onChange={setPassword} placeholder="••••••••" />
                <Field label="Xác nhận mật khẩu" type="password" value={confirmPassword} onChange={setConfirmPassword} placeholder="••••••••" />
